#!/usr/bin/env python3
# mmparse.py -- Parser and Utilities from the Metamath language
# original from mmverify as follows:
...
def vprint(vlevel, *args):
	...
	if verbosity >= vlevel: print(args)
...
class MM:
	...
	def create_tokset(self):
		...
		for ldatak in self.labels:
			...
			if self.labels[ldatak][0] in ('$p', '$a'):
				...
				hyps = pdata[2]
				asserts = pdata[3]
				[tokset.add(tok) for tokp in temp_hyps for tok in tokp]
				if len(hyps) > 0:
	...
	def map_labels(self):
		labelmap = dict()
		labelprops = []
		for itok, tok in enumerate(tokset):
			labelmap[tok] = itok
		for ldatak in self.labels:
			pdata = self.labels[ldatak][1]
			if self.labels[ldatak][0] in ('$p', '$a'):
...
if __name__ == '__main__':
	mm = MM()
	fname = '/devlink/dev/metamath/set.mm'
	fh = open(fname, 'rt')
